...
export class SeriesEventFIX1781121771248 implements MigrationInterface {
  name = 'SeriesEventFIX1781121771248';

  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` DROP COLUMN \`start\``,
    );
    await queryRunner.query(`ALTER TABLE \`seriesevent\` DROP COLUMN \`end\``);
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` ADD \`startTime\` varchar(5) NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` ADD \`endTime\` varchar(5) NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` ADD \`seriesStart\` datetime NOT NULL`,
    );
    await queryRunner.query(
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` DROP COLUMN \`seriesStart\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` DROP COLUMN \`endTime\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` DROP COLUMN \`startTime\``,
    );
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` ADD \`end\` datetime NOT NULL`,
    );
    await queryRunner.query(
      `ALTER TABLE \`seriesevent\` ADD \`start\` datetime NOT NULL`,
    );
  }
}
