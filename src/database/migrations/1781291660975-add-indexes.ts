...
export class AddIndexes1781291660975 implements MigrationInterface {
  name = 'AddIndexes1781291660975';
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    await queryRunner.query(
      `CREATE INDEX \`IDX_CALENDAR_EVENT_DELETED_AT\` ON \`calendarevent\` (\`deletedAt\`)`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_CALENDAR_EVENT_HOLIDAY_LOOKUP\` ON \`calendarevent\` (\`categoryid\`, \`start\`, \`end\`)`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_CALENDAR_EVENT_ROOM_START_END\` ON \`calendarevent\` (\`roomid\`, \`start\`, \`end\`)`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_CALENDAR_EVENT_START\` ON \`calendarevent\` (\`start\`)`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_CALENDAR_EVENT_SERIES\` ON \`calendarevent\` (\`seriesid\`)`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_SERIES_EVENT_LAST_GENERATED\` ON \`seriesevent\` (\`lastGeneratedUntil\`)`,
    );
    await queryRunner.query(
      `CREATE INDEX \`IDX_SERIES_EVENT_ACTIVE\` ON \`seriesevent\` (\`active\`)`,
    );
    await queryRunner.query(
